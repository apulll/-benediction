...
class UserCtl extends Controller {
	...
	async getUserAll(req, res, next) {
		try {
			...
			res
				.status(200)
				.send(jsonFormatter({ msg: "获取用户信息异常" + error }, true));
		}
	}
	/**
	 * 获取用户创建和接收到的祝福总数
	...
	async getUserInfo(req, res, next) {
		try {
			...
			if (results) {
				...
				const ids = getBenisonIds(
					JSON.parse(JSON.stringify(newResults)),
					"bension_id"
				);
				const catalogids = getBenisonIds(
					JSON.parse(JSON.stringify(catalogs)),
					"id"
				);
