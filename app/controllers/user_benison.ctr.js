...
* @Last Modified time: 2018-03-28 22:39:04
*/
	...
	async updateUserBenisonRecords(req, res, next) {
		try {
			...
			if (!errors.isEmpty()) {
				return res
					.status(200)
					.send(jsonFormatter({ errors: errors.array() }, true));
			}

			const data = getDataFromReq(req);

			const userRes = await model.UserModel.findById(data.user_id);
			const benisonRes = await model.BenisonModel.findById(
				data.bension_id
			);
			const userBenRes = await model.UserBenisonModel.findOne({
			...
			if (!benisonRes) {
				return res
					.status(200)
					.send(jsonFormatter({ msg: "祝福语不存在" }, true));
			}
			if (userBenRes) {
				return res
					.status(200)
			...
			if (results) {
				...
				res
					.status(200)
					.send(jsonFormatter({ msg: "写入关系失败" }, true));
			}
		} catch (error) {
			Logger.error(error);
			res
				.status(200)
				.send(jsonFormatter({ msg: "写入关系异常" + error }, true));
		}
	}
}
